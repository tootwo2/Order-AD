...
public class CheckboxGroup extends LinearLayout {
    ...
    // 存放当前的radioButton
    private ArrayList<CheckBox> checkBoxesButtons;
    ...
    private void init() {
        ...
        checkBoxesButtons = new ArrayList<CheckBox>();
    }
    ...
    private void setCheckedView(ViewGroup vg) {
        ...
        for (int i = 0; i < len; i++) {
            if (vg.getChildAt(i) instanceof CheckBox) {// 如果找到了，就设置check状态
                final CheckBox button = (CheckBox) vg.getChildAt(i);
                // 添加到容器
                checkBoxesButtons.add(button);
                if (button.isChecked()) {
                    mProtectFromCheckedChange = true;
    ...
    /** 查找radioButton控件 */
    public CheckBox findRadioButton(ViewGroup group) {
        CheckBox resBtn = null;
        int len = group.getChildCount();
        for (int i = 0; i < len; i++) {
            if (group.getChildAt(i) instanceof CheckBox) {
                resBtn = (CheckBox) group.getChildAt(i);
    ...
    /** 返回当前checkBox控件的count */
    public int getCheckboxCount() {
        return checkBoxesButtons.size();
    }

    /** 返回当前index的checkBox */
    public CheckBox getCheckbox(int index) {
        return checkBoxesButtons.get(index);
    }
    ...
    private class PassThroughHierarchyChangeListener implements
        ...
        public void onChildViewAdded(View parent, View child) {
            ...
            } else if (parent == CheckboxGroup.this
